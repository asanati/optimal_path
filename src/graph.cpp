...
Graph::Graph(const std::vector<std::vector<int>>& grid) {
    ...
    std::vector<std::vector<int>> vec(this->numVertices, std::vector<int> (this->numVertices, 0));
    this->adjMatrix = vec;
...
int Graph::convertTo1D(int i, int j) {
...
std::vector<int> Graph::convertTo3D(int vertex) {
    int x = vertex / (this->numy*this->numz), y = (vertex / (this->numz)) % this->numy, z = vertex % this->numz;
    return {x, y, z};
}

void Graph::setEdges(const std::vector<std::vector<std::vector<int>>>& grid, int i, int j, int k) {
    if (grid[i][j][k] == 1) return;
...
int Graph::minDistanceVertex(std::vector<int> distances, std::vector<bool> visited) {
...
std::vector<int> Graph::dijkstra(int u, int v) {
    ...
    for (int i = 0; i < this->numVertices; ++i) {
        ...
        for (int k = 0; k < this->numVertices; ++k) {
            if (!visited[k] && this->adjMatrix[j][k] != 0 && distances[j] != INT_MAX) {
...
int Graph::heuristic(int u, int v, std::string metric) {
    ...
    if (metric == "euclidean")
        result = sqrt(pow(v_coords[0] - u_coords[0], 2) + pow(v_coords[1] - u_coords[1], 2));
    else
